            ...
            <div class="x_panel">
                <div class="x_content">
                    <form id="form-add-module" data-parsley-validate class="form-horizontal form-label-left">
                       <c:forEach items="${config }" var="moduleSetting">
                            <spring:eval expression="@wdBusinessElementService.selectByPrimaryKey(moduleSetting.businessElementId)" var="wdBusinessElement"/>
                            <wd:baseElement wdBusinessElement="${wdBusinessElement }" defValue="${wdApplicationBusiness.getJsonData()[wdBusinessElement.key]}"/>
                        </c:forEach>
                    </form>
                </div>
            </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#submit-add-module").click(function (event) {
                ...
                var _form_select = $('#form-add-module').find('input,textarea');
                var _haserror = false;
                ...
                $.ajax({
                    url: "${ctx}/wd/application/survey/save_business",
                    async: false,
                    cache: false,
                    type: "POST",
                    data: {
                    	businessId : "${wdApplicationBusiness.id}",
                        applicationId : "${applicationId}",
                        data : JSON.stringify(auditConclusion)
                    },
                    dataType: "json",
                    success: function (result) {
                        if (result.success) {
                            SetLayerData("save_business_data", result.data);
                            CloseIFrame();
                        } else {
