                ...
                <div class="x_content">
                    <form id="form-add-module" data-parsley-validate class="form-horizontal form-label-left">
                        <div class="form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12" for="relationType">
                                                        与申请人关系<span class="required"></span>
                            </label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <select class="form-control" id="relationType" required>
                                    <option value=""> 请选择 </option>
                                    <c:forEach items="${fns:getDictList('person_relation')}" var="personRelation">
                                        <option value="${personRelation.label }" ${wdPersonRelation.relationType eq personRelation.label ? 'selected' : '' }>${personRelation.label }</option>
                                    </c:forEach>
                                </select>
                        ...
                        <c:forEach items="${config }" var="simpleModuleSetting">
                            ...
                            <wd:baseElement wdBusinessElement="${wdBusinessElement }" required="${simpleModuleSetting.required == '1'}" elementSelectListId="${simpleModuleSetting.elementSelectListId }" defValue="${wdPerson.getJsonData()[wdBusinessElement.key] }"/>
                        </c:forEach>
                    </form>
                </div>
            </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#submit-add-module").click(function (event) {
                ...
                $.ajax({
                    url: "${ctx}/wd/application/survey/save_related_person",
                    async: false,
                    cache: false,
                    type: "POST",
                    data: {
                    	relationId: "${wdPersonRelation.id}",
                    ...
                    success: function (result) {
                        if (result.success) {
                            SetLayerData("save_person_relation_data", result.data);
                            CloseIFrame();
                        } else {
