                ...
                <div class="x_content">
                    <form id="form-add-module" data-parsley-validate class="form-horizontal form-label-left">
                        <%-- <div class="form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12" for="applicationPersonRelationId">
                                                        关系人<span class="required"></span>
                            </label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <select class="form-control" id="applicationPersonRelationId" required>
                                    <option value=""> 请选择 </option>
                                    <c:forEach items="${customerRelationList}" var="customerRelation">
                                        <option value="${customerRelation.relationId}" ${wdApplicationRecognizor.applicationPersonRelationId eq customerRelation.relationId ? 'selected' : '' }> ${customerRelation.relationType} - ${customerRelation.getJsonData().base_info_name } </option>
                                    </c:forEach>
                                </select>
                        ...
                        <div class="form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12" for="applicationPersonRelationId">
                                                        房产<span class="required"></span>
                            </label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <select class="form-control" id="originalId" required>
                                    <option value=""> 请选择 </option>
                                    <c:forEach items="${customerBuildingList}" var="customerBuilding">
                                        <option value="${customerBuilding.id}" ${wdApplicationBuildingMortgage.originalId eq customerBuilding.id ? 'selected' : '' }> ${customerBuilding.getJsonData().assets_building_street } </option>
                                    </c:forEach>
                                </select>
                                <div class="warning hide">
                                    <ins class="import"></ins>
                        ...
                        <c:forEach items="${config }" var="simpleModuleSetting">
                            ...
                            <wd:baseElement wdBusinessElement="${wdBusinessElement }" required="${simpleModuleSetting.required == '1'}" elementSelectListId="${simpleModuleSetting.elementSelectListId }" defValue="${wdApplicationRecognizor.getJsonData()[wdBusinessElement.key] }"/>
                        </c:forEach>
                    </form>
                </div>
            </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#submit-add-module").click(function (event) {
                ...
                $.ajax({
                    url: "${ctx}/wd/application/survey/save_building_mortgage",
                    async: false,
                    cache: false,
                    type: "POST",
                    data: {
                    	id: "${wdApplicationBuildingMortgage.id}",
                    ...
                    success: function (result) {
                        if (result.success) {
                            SetLayerData("reload_survey_view", true);
                            CloseIFrame();
                        } else {
