                ...
                <div class="x_content">
                    <form id="form-add-module" data-parsley-validate class="form-horizontal form-label-left">
                        <c:forEach items="${config }" var="simpleModuleSetting">
                            ...
                            <wd:baseElement wdBusinessElement="${wdBusinessElement }" required="${simpleModuleSetting.required == '1'}" elementSelectListId="${simpleModuleSetting.elementSelectListId }" defValue="${personAssetsCar.getJsonData()[wdBusinessElement.key] }"/>
                        </c:forEach>
                    </form>
                </div>
            </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#submit-add-module").click(function (event) {
                ...
                $.ajax({
                    url: "${ctx}/wd/application/survey/add_person_assets_car",
                    async: false,
                    cache: false,
                    type: "POST",
                    data: {
                    	id: "${personAssetsCar.id}",
                    ...
                    success: function (result) {
                        if (result.success) {
                            SetLayerData("save_person_car_data", result.data);
                            CloseIFrame();
                        } else {
