...
<ul id="new_${wdPersonRelation.id }">
    <li><label>与申请人关系：</label>
        <p>${wdPersonRelation.relationType }
            <span class="delete del_application_coborrower" data-id="${wdApplicationCoborrower.id }"></span>
            <span class="edit edit_application_coborrower" data-id="${wdApplicationCoborrower.id }"></span>
         </p>
    </li>
    ...
    <c:forEach items="${config}" var="productConfig">
        <spring:eval expression="@wdBusinessElementService.selectByPrimaryKey(productConfig.businessElementId)" var="wdBusinessElement"/>
         <li>
            ...
            <c:choose>
                <c:when test="${fns:getDataCategory(wdBusinessElement) == 1}">
                   <wd:picList dataList="${wdApplicationCoborrower.getJsonData()[wdBusinessElement.key]}" />
                </c:when>
                <c:otherwise>
                    <p>${wdApplicationCoborrower.getJsonData()[wdBusinessElement.key] }</p>
                </c:otherwise>
            </c:choose>
        </li>
    </c:forEach>
