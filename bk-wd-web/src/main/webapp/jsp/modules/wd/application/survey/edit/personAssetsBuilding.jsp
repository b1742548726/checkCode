                ...
                <div class="x_content">
                    <form id="form-add-module" data-parsley-validate class="form-horizontal form-label-left">
                        <c:forEach items="${config }" var="simpleModuleSetting">
                            ...
                            <wd:baseElement wdBusinessElement="${wdBusinessElement }" required="${simpleModuleSetting.required == '1'}" elementSelectListId="${simpleModuleSetting.elementSelectListId }" defValue="${personAssetsBuilding.getJsonData()[wdBusinessElement.key] }"/>
                        </c:forEach>
                    </form>
                </div>
            </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#submit-add-module").click(function (event) {
                ...
                $.ajax({
                    url: "${ctx}/wd/application/survey/add_person_assets_building",
                    async: false,
                    cache: false,
                    type: "POST",
                    data: {
                    	id: "${personAssetsBuilding.id}",
                    ...
                    success: function (result) {
                        if (result.success) {
                            SetLayerData("save_person_assets_building_data", result.data);
                            CloseIFrame();
                        } else {
