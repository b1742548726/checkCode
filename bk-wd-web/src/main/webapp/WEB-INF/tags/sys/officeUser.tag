...
<c:forEach items="${dataList }" var="data">
    <li class="reduce">
        ...
        <c:choose>
            <c:when test="${not empty data.childList}">
                <ul>
                    <sys:officeUser dataList="${data.childList}" box="${box }" edit="${edit }" add="${add }" remove="${remove }"/>
                </ul>
            </c:when>
            <c:otherwise>
                <spring:eval expression="@sysUserService.selectByOfficeId(data.id)" var="userList"/>
                <ul>
                    <c:forEach items="${userList }" var="sysuser">
                        <li class="user">
                            <p data-id="${sysuser.id }" data-type="user">
